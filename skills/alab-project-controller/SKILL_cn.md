...
- 如果 worker 需要 shared ignored run files，只用 `--add-dir` 加入该目录；除非任务明确需要，不要加入 ALab home。
...
- 在 experiment worktrees 中启动 worker agents，但不传递 project admin 或 root credentials。只提供任务说明和非 secret helper variables；让 worker 使用自己的 worktree token 执行 `alab run` 和 `alab submit`。

...
- When a project uses free evaluation (`runner.type = "none"` and `reward.type = "none"`), tell workers to submit directly without `alab run`; final run id will be `none` and the result will not appear in best reward ranking.

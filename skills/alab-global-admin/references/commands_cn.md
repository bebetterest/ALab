...
- **`config show`**：查看 ALab home-level config 和 validity。
  关键参数：无必需参数。
  注意点：显示 schema version、output format、preview bytes、lock timing、busy timeout 和 config validity。
- **`config set`**：修改一个 ALab home-level setting。
  关键参数：必需 `<field> <toml-literal>`；允许 output、storage timeout 和 lock timing 相关 fields。
...
- **`key list --project`**：查看 project admin credential rows。
...
- **`context show`**：查看 path context markers 和 registered ALab path records。
  关键参数：可选 `--path <dir>`；默认 `.`。
...
- **`project init`**：创建 project、source、config version、baseline validation 和 project admin key。
  ...
  注意点：Runtime behavior 只来自 config。Generated project admin key 在 project row 创建后只打印一次。
- **`project list|show`**：查看 retained projects。
...
- **`project remove`**：以 audited cascade 删除 archived project 及其 retained tree。
  关键参数：必需 `--cascade`；再加 `--dry-run` 或 `--force --confirm <project_id>`；可选 `--project`、`--reason`。
  注意点：Root-only 且 destructive。始终先 dry-run；actual remove 会把 filesystem paths stage 到 ALab trash。
- **`catalog skydiscover add|update`**：安装或修改 pinned SkyDiscover catalog。
  关键参数：可选 `--origin-url <url>`；`--ref <ref>` 和 `--commit <full_sha>` 至多二选一。
  注意点：可复现实例和 release 优先用 `--commit`。`update` 要求 ALab-managed catalog 干净。
- **`catalog skydiscover show`**：查看 active SkyDiscover catalog metadata。
  关键参数：不需要 catalog selector。
  注意点：不应访问网络；用于确认 pinned commit 和 local path。
- **`catalog skydiscover remove`**：删除 active SkyDiscover catalog metadata 和 local checkout。
...
- **`audit show`**：查看单个 audit event 和 sanitized metadata。

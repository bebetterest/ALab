...
ALab is a local agent-first experiment workbench. It separates root home administration, project coordination, and experiment worktree implementation so agents can collaborate without sharing unnecessary credentials or mutating the wrong layer.
...
- When creating or continuing experiment work, create the project/experiment at the higher layer first, then hand implementation to a separate session/thread or subagent using the matching lower-layer skill and only the matching credential/token context.

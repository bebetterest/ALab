...
Initializes an isolated ALab home and free-evaluation project. Secrets are
written only under ignored .run/secrets/.
EOF
      exit 0
      ;;
    *)
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would initialize the Free Evaluation Intro Site example.
...
{
  printf '# Generated by free_evaluation_intro_site/scripts/setup_project.sh\n'

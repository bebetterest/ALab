...
Creates one SkyDiscover Python TSP experiment, enables the 2-opt improvement,
...
(cd "$WORKTREE_PATH" && run_alab run --message "tsp skydiscover python 2-opt improvement") | tee "$RUN_LOG"
(cd "$WORKTREE_PATH" && run_alab submit --message "tsp skydiscover python submitted route" --summary "Enabled deterministic 2-opt route improvement." --feedback "The latest SkyDiscover Python run passed with parsed metrics." --ref none) | tee "$SUBMIT_LOG"

...
Creates one SkyDiscover Python TSP experiment, enables a nearest-neighbor
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would run the SkyDiscover Python TSP template demo.
...
CREATE_LOG="$LOG_DIR/tsp-skydiscover-python-exp-create.log"
RUN_LOG="$LOG_DIR/tsp-skydiscover-python-run.log"
SUBMIT_LOG="$LOG_DIR/tsp-skydiscover-python-submit.log"
...
(cd "$WORKTREE_PATH" && run_alab run --message "tsp skydiscover python nearest-neighbor improvement") | tee "$RUN_LOG"
(cd "$WORKTREE_PATH" && run_alab submit --message "tsp skydiscover python submitted route" --summary "Enabled deterministic nearest-neighbor route improvement." --feedback "The latest SkyDiscover Python run passed with parsed minimize-direction metrics." --ref none) | tee "$SUBMIT_LOG"

RUN_ID="$(extract_field "run id" "$RUN_LOG")"
REWARD_VALUE="$(extract_field "reward value" "$RUN_LOG")"
cat > "$REPORT_DIR/report.md" <<EOF
# TSP SkyDiscover Python Template Report

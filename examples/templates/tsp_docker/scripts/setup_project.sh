...
Initializes the Docker TSP template with an isolated ALab home.
EOF
      exit 0
      ;;
    *)
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would initialize the Docker TSP template.

Template:    $TEMPLATE_DIR
ALAB home:   $ALAB_EXAMPLE_HOME
Project env: $PROJECT_ENV
Dockerfile:  $TEMPLATE_DIR/source/Dockerfile
ALAB repo:   $REPO_ROOT
...
PROJECT_OUTPUT="$(run_alab --key "$ROOT_KEY" project init local --config "$TEMPLATE_DIR/alab.project.toml" --source-path "$TEMPLATE_DIR/source")"
...
{
  printf '# Generated by examples/templates/tsp_docker/scripts/setup_project.sh\n'

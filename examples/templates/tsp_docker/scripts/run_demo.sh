...
Creates one Docker TSP experiment, enables a nearest-neighbor improvement, runs
...
EXP_NAME="${EXP_NAME:-tsp-docker-demo}"
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would run the Docker TSP template demo.
...
CREATE_LOG="$LOG_DIR/tsp-docker-exp-create.log"
RUN_LOG="$LOG_DIR/tsp-docker-run.log"
SUBMIT_LOG="$LOG_DIR/tsp-docker-submit.log"
...
(cd "$WORKTREE_PATH" && run_alab run --message "tsp docker nearest-neighbor improvement") | tee "$RUN_LOG"
(cd "$WORKTREE_PATH" && run_alab submit --message "tsp docker submitted route" --summary "Enabled deterministic nearest-neighbor route improvement." --feedback "The latest Docker run passed with parsed minimize-direction file reward." --ref none) | tee "$SUBMIT_LOG"

RUN_ID="$(extract_field "run id" "$RUN_LOG")"
REWARD_VALUE="$(extract_field "reward value" "$RUN_LOG")"
cat > "$REPORT_DIR/report.md" <<EOF
# TSP Docker Template Report

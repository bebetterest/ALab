...
Creates one Docker TSP experiment, enables the 2-opt improvement, runs it, and
...
(cd "$WORKTREE_PATH" && run_alab run --message "tsp docker 2-opt improvement") | tee "$RUN_LOG"
(cd "$WORKTREE_PATH" && run_alab submit --message "tsp docker submitted route" --summary "Enabled deterministic 2-opt route improvement." --feedback "The latest Docker run passed with parsed file reward." --ref none) | tee "$SUBMIT_LOG"

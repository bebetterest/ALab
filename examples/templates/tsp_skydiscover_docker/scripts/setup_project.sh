...
Initializes the SkyDiscover Docker TSP template with an isolated ALab home.
EOF
      exit 0
      ;;
    *)
...
EVALUATOR_DIR="$TEMPLATE_DIR/evaluator"
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would initialize the SkyDiscover Docker TSP template.
...
PROJECT_OUTPUT="$(run_alab --key "$ROOT_KEY" project init skydiscover --config "$CONFIG_PATH" --source-path "$TEMPLATE_DIR/source")"
...
{
  printf '# Generated by examples/templates/tsp_skydiscover_docker/scripts/setup_project.sh\n'
...
# TSP SkyDiscover Docker Template Setup

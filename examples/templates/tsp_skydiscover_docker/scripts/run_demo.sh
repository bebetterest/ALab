...
Creates one SkyDiscover Docker TSP experiment, enables a nearest-neighbor
improvement, runs it, and submits the passed result.
EOF
      exit 0
      ;;
    *)
...
EXP_NAME="${EXP_NAME:-tsp-skydiscover-docker-demo}"
UV_DEFAULT_INDEX="${UV_DEFAULT_INDEX:-https://pypi.org/simple}"

if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would run the SkyDiscover Docker TSP template demo.
...
CREATE_LOG="$LOG_DIR/tsp-skydiscover-docker-exp-create.log"
RUN_LOG="$LOG_DIR/tsp-skydiscover-docker-run.log"
SUBMIT_LOG="$LOG_DIR/tsp-skydiscover-docker-submit.log"
...
(cd "$WORKTREE_PATH" && run_alab run --message "tsp skydiscover docker nearest-neighbor improvement") | tee "$RUN_LOG"
(cd "$WORKTREE_PATH" && run_alab submit --message "tsp skydiscover docker submitted route" --summary "Enabled deterministic nearest-neighbor route improvement." --feedback "The latest SkyDiscover Docker run passed with parsed minimize-direction metrics." --ref none) | tee "$SUBMIT_LOG"

RUN_ID="$(extract_field "run id" "$RUN_LOG")"
REWARD_VALUE="$(extract_field "reward value" "$RUN_LOG")"
cat > "$REPORT_DIR/report.md" <<EOF
# TSP SkyDiscover Docker Template Report

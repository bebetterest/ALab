...
Creates one SkyDiscover Docker TSP experiment, enables the 2-opt improvement,
runs it, and submits the passed result.
EOF
      exit 0
      ;;
    *)
...
(cd "$WORKTREE_PATH" && run_alab run --message "tsp skydiscover docker 2-opt improvement") | tee "$RUN_LOG"
(cd "$WORKTREE_PATH" && run_alab submit --message "tsp skydiscover docker submitted route" --summary "Enabled deterministic 2-opt route improvement." --feedback "The latest SkyDiscover Docker run passed with parsed metrics." --ref none) | tee "$SUBMIT_LOG"

...
Initializes a local ALab project for collaboration, observe, and lifecycle
commands.
EOF
      exit 0
      ;;
...
INSPECTION_ROOT="$RUN_DIR/inspection"
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would initialize the collaboration example.
...
mkdir -p "$SECRET_DIR" "$LOG_DIR" "$REPORT_DIR" "$WORKTREE_ROOT" "$INSPECTION_ROOT" "$UV_CACHE_DIR" "$PYTHONPYCACHEPREFIX"
...
PROJECT_OUTPUT="$(run_alab --key "$ROOT_KEY" project init local --config "$EXAMPLE_DIR/alab.project.toml" --source-path "$EXAMPLE_DIR/source")"
...
{
  printf '# Generated by collaboration_observe_lifecycle/scripts/setup_project.sh\n'
  ...
  printf 'export ALAB_INSPECTION_ROOT=%q\n' "$INSPECTION_ROOT"
} > "$PROJECT_ENV"

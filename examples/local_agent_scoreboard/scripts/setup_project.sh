...
Initializes an isolated ALab home and local-runner project. Secrets are written
only under ignored .run/secrets/.
EOF
      exit 0
      ;;
    *)
...
if [[ "$DRY_RUN" == "1" ]]; then
  cat <<EOF
Would initialize the Local Agent Scoreboard example.
...
Commands:
  ...
  $ALAB_CMD_PREFIX --key <root-key> project init local --config $CONFIG_PATH --source-path $SOURCE_PATH
EOF
  exit 0
fi
...
mkdir -p "$SECRET_DIR" "$SHARED_DIR" "$LOG_DIR" "$REPORT_DIR" "$WORKTREE_ROOT" "$UV_CACHE_DIR" "$PYTHONPYCACHEPREFIX"
...
PROJECT_OUTPUT="$(run_alab --key "$ROOT_KEY" project init local --config "$CONFIG_PATH" --source-path "$SOURCE_PATH")"
...
{
  printf '# Generated by local_agent_scoreboard/scripts/setup_project.sh\n'
...
- Secret env: \`.run/secrets/project.env\`
EOF

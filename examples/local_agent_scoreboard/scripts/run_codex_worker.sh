...
Experiment:  $EXP_NAME
Command:
  env -u ALAB_PROJECT_KEY -u ALAB_ROOT_KEY -u ALAB_KEY ALAB_CMD_PREFIX="\$ALAB_CMD_PREFIX" codex exec -C <worktree> --add-dir "\$ALAB_EXAMPLE_HOME" --add-dir "\$UV_CACHE_DIR" --add-dir "\$PYTHONPYCACHEPREFIX" --add-dir "$SHARED_DIR" --sandbox workspace-write - < $EXAMPLE_DIR/prompts/worker.md
EOF
  exit 0
fi

...
- 更新 P0 full default-suite evidence row，使其指向最新的 2026-05-23 examples/reward-parser 后 full-suite run，而不是较早的 2026-05-22 CLI audit closeout run。
- 更新 dashboard 和 active-pipeline 的日期/current wording，并同步中文 counterparts。
- 明确本批次只调整文档措辞，没有新增 implementation 或 test changes，因此 2026-05-23 full-suite gate 仍是最新 implementation/test gate。

验证：

- Focused docs sync：`UV_CACHE_DIR=/private/tmp/alab-uv-cache UV_DEFAULT_INDEX=https://pypi.org/simple PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv run --locked pytest tests/test_cli_contract.py::test_root_and_docs_markdown_files_have_synchronized_chinese_pairs tests/test_cli_contract.py::test_selected_english_and_chinese_success_fields_are_synchronized -q`
- `git diff --check`
- `rg -n "[ \t]+$" docs/completion_audit.md docs/completion_audit_cn.md docs/progress.md docs/progress_cn.md docs/progress_pipeline.md docs/progress_pipeline_cn.md docs/progress_log.md docs/progress_log_cn.md` 无匹配。

## 2026-05-23 Docs Directory README

已实现：

- 新增 [docs/README.md](README.md) 作为 documentation directory guide，覆盖 default read order、document groups 和 update rules。
- 新增同步的 [docs/README_cn.md](README_cn.md)。
- 更新根 README/README_cn 的 repository tree 和 documentation section，指向 docs guide。

验证：

- Focused docs sync 和 README structure tests：`UV_CACHE_DIR=/private/tmp/alab-uv-cache UV_DEFAULT_INDEX=https://pypi.org/simple PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv run --locked pytest tests/test_cli_contract.py::test_root_and_docs_markdown_files_have_synchronized_chinese_pairs tests/test_cli_contract.py::test_readme_repository_structure_trees_are_synchronized_and_existing tests/test_cli_contract.py::test_selected_english_and_chinese_success_fields_are_synchronized -q`
- `git diff --check`
- `rg -n "[ \t]+$" README.md README_cn.md docs/README.md docs/README_cn.md docs/progress_log.md docs/progress_log_cn.md docs/completion_audit.md docs/completion_audit_cn.md docs/progress.md docs/progress_cn.md docs/progress_pipeline.md docs/progress_pipeline_cn.md` 无匹配。

## 2026-05-23 TSP 模板库

已实现：

- 新增 `examples/templates/` 作为可复制的默认 TSP template suite，覆盖 `tsp_local`、`tsp_docker`、`tsp_harbor`、`tsp_skydiscover_python` 和 `tsp_skydiscover_docker`。
- 每个模板都包含完整 project config 或 generated-config template、starter `solution.py`、deterministic city data、validation/evaluator logic、`scripts/setup_project.sh`、`scripts/run_demo.sh` 和 ignored `.run/` state。
- 统一 TSP contract：`build_route(cities) -> list[int]`、numeric-only reward metrics、deterministic 2-opt demo improvement、route artifacts/feedback、hidden Harbor verifier diagnostics，以及 adapter templates 的 local path injection。
- 更新 root 与 examples README pairs，加入 template matrix 和使用路径。
- 对齐 SkyDiscover adapter config preflight 与 runner resolution，使 local evaluator refs 不需要 SkyDiscover catalog checkout 也可工作。
- 新增 contract tests，覆盖 template completeness、shell syntax、dry-run behavior、local SkyDiscover evaluator refs、从临时副本真实运行默认 local/SkyDiscover Python templates，以及 opt-in real Docker template runs。
- 更新 completion audit 的 SkyDiscover project-init row，加入新的 local evaluator-ref evidence。
...
- 在每个 `examples/templates/tsp_*` 目录下新增 per-template README/README_cn，使只复制单个 template 时仍保留本地命令路径、可编辑文件表、runner requirements，以及 `.run/`/secret-state 约定。
- 更新顶层 template README pair，说明 per-template README 文件以及 `ALAB_REPO_ROOT`/`ALAB_BIN` 的路径 quoting 行为。
- 重写所有 TSP template setup/run scripts 的 ALab command 构造方式：默认命令使用 Bash array，并把生成的 command array 持久化到 `.run/secrets/project.env`，从而支持包含空格的 copied template path 和 custom wrapper path，不破坏参数边界。
- 扩展 template contract tests，要求每个本地 README pair 存在，并在包含空格的 copied template path 和 `ALAB_BIN` wrapper path 下真实运行默认 local/SkyDiscover Python template demo。
...
- 新增顶层 `alab report [--project <project_id>] [--exp <exp_id>] --out <path> [--overwrite]`，primary object type 为 `report`。
- Project reports 需要 root/admin authority；experiment reports 可使用 root/admin，或能 observe target experiment 的 experiment/inspection token。
- 生成的 Markdown reports 包含安全的 project/experiment metadata、counts、recent run/result tables、submission summary/feedback 和 artifact/log metadata，同时省略 raw keys、raw tokens、raw `secret_env` values、hidden log contents 和 artifact bytes。
- 为 top-level project、experiment、run、log、artifact 和 audit APIs 增加 pagination metadata 与有界 `limit`/`offset` handling，同时保持 feedback capture 不变。
- 新增 top-level `/api/logs` 和 `/api/artifacts`、有界 high-volume project/experiment/run detail related arrays、cache-entry paging，以及 frontend loaded/total metadata，使 global asset views 不再 fetch every project detail。
- 按本 batch 明确范围，保持 `alab feedback`、JSON output 和 local batch execution 不变。
- 更新 README、blueprint、CLI/dashboard/test specs、role skills、completion audit、progress dashboard/pipeline/guardrails，并同步中文文档。
...
- 新增同步的 `CHANGELOG.md` 和 `CHANGELOG_cn.md`，记录 version `0.1.2` release notes。
- 新增 PyPI project metadata 的 `Changelog` URL，让 PyPI 用户可以找到面向 release 的变更记录。
- 更新 `publish-python` GitHub Actions job：成功发布新的 PyPI version 后，会创建 `v<version>` GitHub Release，并使用 `CHANGELOG.md` 中匹配版本段落作为 release notes。
- 当 PyPI 已有当前 `pyproject.toml` version 时，保持原有 publish skip behavior 不变。
- 更新 README/README_cn 中的 release workflow 和 repository layout 说明。
...
- 新增 schema migration `3_free_evaluation.sql`，将 runtime schema version 升到 3，使 `experiment_submissions.final_run_id` 可为 null，并为 project config versions 和 project validations 增加 `not_required` validation status。
- 新增成对的 `runner.type = "none"` 和 `reward.type = "none"` config 支持，包括拒绝 none/non-none 混用、拒绝 `runner.type = "none"` 上的 executable runner fields，以及拒绝 `reward.type = "none"` 上的 reward extractor fields。
- 新增 free project init/import/validate behavior：写入 `not_required` validation rows、将 config 设为 active valid，并跳过 baseline evaluator execution。
- 更新 experiment create、run 和 submit flows：free experiments 会把 workers 指向 direct submit，`alab run` 以 `COMMAND_UNAVAILABLE` 失败，`submit --rerun` 以 `CONFIG_INVALID` 失败，存储 `final_run_id = NULL`，渲染 `final run id: none`，不创建 run/log/artifact/reward rows，并保持 mutable-scope、secret、ref visibility、archive/remove、report、observe 和 best-ranking behavior 一致。
- 同步更新 README、blueprint/spec docs、role skill references、changelogs、progress 和 completion audit 的中英文内容。

...
Current active focus, in summary only: the 2026-06-02 feedback lifecycle batch has landed, and the current object-family `services.py` extraction batch is now closed. Feedback submit/list/show/archive handlers live in `src/alab/feedback.py`; audit list/show handlers and audit object-id filtering live in `src/alab/audit.py`; the dashboard command handler lives with the dashboard server/read models in `src/alab/dashboard.py`; SkyDiscover catalog add/update/show/remove handlers and catalog/adapter-ref resolution helpers live in `src/alab/catalog.py`; annotation add/edit/archive/unarchive/remove plus observe annotation list/show handlers live in `src/alab/annotations.py`; observe run/artifact/log list/show/export/archive/unarchive/remove handlers live in `src/alab/observe.py`; Markdown report export lives in `src/alab/report.py`; source import/list/show/archive/unarchive/remove handlers live in `src/alab/sources.py`; shared audit-event writing lives in `src/alab/service_audit.py`; shared request auth gates live in `src/alab/service_auth.py`; and shared text/reason validation lives in `src/alab/service_text.py`. Registered behavior for those CLI surfaces remains unchanged. Project, experiment, run/submit, context, config, credential, and shared lifecycle helpers intentionally remain in `src/alab/services.py` until their shared helper boundaries can be moved without creating fragile cross-module imports. Broad all-at-once service-module extraction and new runner families remain out of scope. Recently closed proof families are tracked in `docs/progress_closed_gaps.md`; do not restart them unless `docs/completion_audit.md` names a new concrete edge or the release target differs from the currently proved host/platform/upstream gates. The detailed queue state lives in `docs/progress_pipeline.md`.
...
- The latest full default-suite closeout gate passed after the 2026-06-03 annotation, observe, report, and source extraction changes. The sandbox full suite failed only because dashboard tests bind loopback ports that the managed sandbox rejected with `PermissionError`; the elevated full default suite passed.

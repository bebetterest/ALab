...
- Context repair guardrails：moved experiment worktrees 需要 registered branch 才能 self-token repair；moved inspection checkouts 需要 pinned inspection commit，mismatch 时会保留 registry/audit state，成功 self-token repair 后记录 token-actor repair metadata。

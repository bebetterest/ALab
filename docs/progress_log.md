...
- Added top-level `alab dashboard [--port <0-65535>] [--no-open] [--refresh-seconds <0-3600>]` as a root-only long-running command.
- Added a constrained `127.0.0.1` `ThreadingHTTPServer` dashboard server with random browser-token URL fragments, `X-ALab-Dashboard-Token` API checks, `405` rejection for non-GET/HEAD methods, no directory serving, CSP/no-store headers, and read-only JSON/binary routes.
- Added dashboard read models for global summary, projects, project detail, experiments, runs, logs, artifacts, audit, feedback, and system state, with raw key/token/verifier/salt and raw `secret_env` redaction.
- Added packaged static assets under `src/alab/dashboard_static/`: responsive English/Chinese UI, cards, filters, charts, detail panels, log/artifact preview/download flows, vendored Chart.js, selected Lucide SVG symbols, and license notices.
- Added `LongRunningResult` handling so `cli.run()` can render startup blocks, flush stdout, and then serve the dashboard without changing normal command result behavior.
- Updated README, blueprint, CLI/dashboard/test specs, completion audit, progress/pipeline/guardrails/log, ignored local AGENTS notes, and global-admin skill docs with synchronized Chinese versions.
...
- Browser smoke against a seeded temporary `alab dashboard --no-open --refresh-seconds 0` server: verified Overview render with charts, Projects table data, Chinese language toggle, Experiments view, URL fragment token cleanup, and no visible errors.
...
- Updated the P0 full default-suite evidence row in `docs/completion_audit.md` to reflect the current 2026-05-31 full-suite pass instead of the older stale-after-2026-05-30 wording.
- Updated the P0 documentation consistency row to point at the current cleanup and focused documentation checks.
- Added the 2026-05-31 observe SQL pushdown, experiment list/search/best SQL-backed query optimization, dashboard feedback pagination/search, and runtime diagnostic UX changes to the `0.1.2` changelog section.
- Synchronized the matching Chinese documents.
...
- Moved feedback submit/list/show/archive handlers and feedback file-record helpers from `src/alab/services.py` into `src/alab/feedback.py` without changing registered CLI behavior.
- Moved shared request auth gates into `src/alab/service_auth.py` and shared text input/reason validation helpers into `src/alab/service_text.py`.
- Updated `src/alab/registry.py` to register feedback commands from the extracted object-family module.
- Updated CLI contract static scan helpers to derive command-handler modules from `src/alab/registry.py`, so registered command option/guard/typed-value checks cover both the legacy `services.py` module and extracted service modules.
- Updated progress/audit/pipeline documentation and synchronized Chinese documents.
...
- Rechecked the open workspace diff after the credentials, maintenance, and removal helper extraction batch.
- Confirmed `src/alab/registry.py` registers the extracted auth/key and backup/cache handlers from their object-family modules.
- Confirmed `tests/test_smoke.py` patches Docker cache pruning through `src/alab/maintenance.py` and the warning-code catalog scan now covers runtime modules under `src/alab/*.py`.
- Confirmed remaining `src/alab/services.py` content is still the tightly coupled config/project/experiment/run/submit/context/lifecycle core and should not be split further only to reduce line count.
- Updated progress and completion-audit English/Chinese documents to reflect the 2026-06-04 workspace-review validation.
...
- Moved backup prune and cache prune command handlers from `src/alab/services.py` into `src/alab/maintenance.py`.
- Moved retention parsing and cache cutoff helpers with the handlers so backup/cache option validation remains local to the maintenance command family.
- Updated `src/alab/registry.py` to register maintenance commands from the extracted module.
- Updated the CLI warning-code catalog test to scan runtime modules under `src/alab/*.py` instead of a stale hand-written service-file list.
- Kept shared safe path deletion and trash-cache deletion helpers in `src/alab/removal.py` so maintenance commands reuse the same filesystem safety boundary as lifecycle removals.
- Kept project, experiment, run/submit, context, config, and tightly coupled lifecycle handlers in `src/alab/services.py`; the current object-family extraction is closed rather than continuing into line-count-only splits.
...
- Moved audit list/show command handlers and audit object-id filter validation from `src/alab/services.py` into `src/alab/audit.py` without changing registered CLI behavior.
- Updated `src/alab/registry.py` to register audit commands from the extracted object-family module.
- Kept shared request auth, parser helpers, audit JSON sanitization contracts, and SQLite read behavior unchanged.
- Updated progress/audit/pipeline documentation and synchronized Chinese documents.
...
- Moved `cmd_dashboard` from `src/alab/services.py` into `src/alab/dashboard.py`, colocating the dashboard command handler with the dashboard server and read models.
- Updated `src/alab/registry.py` to register the dashboard command from the extracted dashboard module.
- Updated CLI, registry, and focused dashboard/auth tests to import request/result models directly from `src/alab/service_models.py` instead of relying on `services.py` as a model re-export boundary.
- Updated progress/audit/pipeline documentation and synchronized Chinese documents.
...
- Moved SkyDiscover catalog add/update/show/remove command handlers and catalog/adapter-ref resolution helpers from `src/alab/services.py` into `src/alab/catalog.py` without changing registered CLI behavior.
- Moved shared audit-event insertion into `src/alab/service_audit.py` so extracted handler modules do not need to import back into `services.py`.
- Updated `src/alab/registry.py` to register catalog commands from the extracted catalog module while keeping registry-derived CLI contract scans as the handler source of truth.
- Kept `alab.services` compatibility exports for CLI option constants and migration JSON contract helpers that existing tests read directly.
- Updated progress/audit/pipeline/closed-gap documentation and synchronized Chinese documents.
...
- Moved shared path presence, trash planning, trash staging/restore/finalize, pending trash cleanup, worktree dirty-state, and safe prune helpers from `src/alab/services.py` into `src/alab/removal.py`.
- Kept same-name compatibility imports in `src/alab/services.py` for existing tests and callers that still reach these private helpers through the legacy service module.
- Updated `src/alab/observe.py` to import trash/remove helpers directly from `src/alab/removal.py`, reducing reverse dependency on `services.py`.
- Kept project, experiment, run/submit, context, config, credential, and tightly coupled lifecycle handlers in `src/alab/services.py` to avoid line-count-only over-splitting.
- Updated progress/audit/pipeline/closed-gap documentation and synchronized Chinese documents.
...
- Moved auth init/root-regenerate and key create/list/revoke command handlers from `src/alab/services.py` into `src/alab/credentials.py`.
- Updated `src/alab/registry.py` to register auth/key commands from the extracted credentials module.
- Kept small credential-local project/id lookup helpers inside `src/alab/credentials.py` instead of importing back into `services.py`.
- Kept project, experiment, run/submit, context, config, and tightly coupled lifecycle handlers in `src/alab/services.py` to avoid broad line-count-only extraction.
- Updated progress/audit/pipeline/closed-gap documentation and synchronized Chinese documents.
...
- Moved project config/env/secret command handlers and their local config/secret helpers from `src/alab/services.py` into `src/alab/project_config.py`.
- Moved project validation create/archive/unarchive/remove handlers and validation-row/blocker helpers into `src/alab/project_validation.py`.
- Moved experiment list/search/show/best handlers and visibility/query/best-ranking helpers into `src/alab/experiment_query.py`, including the observe experiment aliases registered to the same handlers.
- Moved experiment archive/unarchive/remove handlers and experiment-remove filesystem target helpers into `src/alab/experiment_lifecycle.py`.
- Moved experiment worktree remove/restore, token list/revoke/regenerate, and inspection checkout create/remove handlers plus their Git/path/token helpers into `src/alab/experiment_access.py`.
- Updated `src/alab/registry.py` to register those command families directly from the extracted modules.
- Kept legacy `src/alab/services.py` compatibility wrappers for existing internal callers and tests that still import private helpers through the historical services module.
- Kept project init, source bootstrap shared with experiment creation, global config, context, run/submit, project list/show/archive/remove/locks, experiment create/tags, and remaining core lifecycle helpers in `src/alab/services.py` to avoid line-count-only over-splitting.
- Updated progress/audit/pipeline/closed-gap documentation and synchronized Chinese documents.

Validation:

- Registry binding check: confirmed `project config/env/secret` commands bind to `alab.project_config`, `project validate/validation` commands bind to `alab.project_validation`, `exp list/search/show/best` plus observe experiment aliases bind to `alab.experiment_query`, `exp archive/unarchive/remove` binds to `alab.experiment_lifecycle`, and worktree/token/checkout commands bind to `alab.experiment_access`.
...
## 2026-06-05 Free Evaluation Mode

Implemented:

- Added schema migration `3_free_evaluation.sql`, raised the runtime schema version to 3, made `experiment_submissions.final_run_id` nullable, and added `not_required` validation status support for project config versions and project validations.
- Added paired `runner.type = "none"` and `reward.type = "none"` config support, including schema rejection for mixed none/non-none configs, executable runner fields on `runner.type = "none"`, and reward extractor fields on `reward.type = "none"`.
- Added free project init/import/validate behavior that records `not_required` validation rows, marks the config active valid, and skips baseline evaluator execution.
- Updated experiment create, run, and submit flows so free experiments point workers to direct submit, reject `alab run` with `COMMAND_UNAVAILABLE`, reject `submit --rerun` with `CONFIG_INVALID`, store `final_run_id = NULL`, render `final run id: none`, skip run/log/artifact/reward rows, and keep mutable-scope, secret, ref visibility, archive/remove, report, observe, and best-ranking behavior consistent.
- Updated README, blueprint/spec docs, role skill references, changelogs, progress, and completion audit in English and Chinese.

Validation:

- Ruff: `UV_CACHE_DIR=/private/tmp/alab-uv-cache PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv --no-config run --locked ruff check src/alab tests`
- Focused free/migration/schema/smoke checks: `UV_CACHE_DIR=/private/tmp/alab-uv-cache PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv --no-config run --locked pytest tests/test_runner_local.py::test_project_config_schema_maps_runner_reward_and_env_edges tests/test_migrations.py::test_migrate_records_exact_file_checksum tests/test_migrations.py::test_representative_ddl_enum_checks_are_enforced tests/test_migrations.py::test_project_source_and_experiment_lifecycle_ddl_contract_checks_are_enforced tests/test_migrations.py::test_audit_secret_submission_and_tag_ddl_contract_checks_are_enforced tests/test_smoke.py::test_free_evaluation_project_allows_direct_submit_without_runs tests/test_smoke.py::test_config_source_observe_and_tags -q`
- Focused docs/CLI contract checks: `UV_CACHE_DIR=/private/tmp/alab-uv-cache PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv --no-config run --locked pytest tests/test_cli_contract.py::test_root_and_docs_markdown_files_have_synchronized_chinese_pairs tests/test_cli_contract.py::test_readme_repository_structure_trees_are_synchronized_and_existing tests/test_cli_contract.py::test_selected_english_and_chinese_success_fields_are_synchronized tests/test_cli_contract.py::test_registered_command_success_field_contracts_are_synchronized tests/test_cli_contract.py::test_registered_commands_have_success_field_contracts_in_cli_specs tests/test_cli_contract.py::test_error_code_catalog_and_numeric_exit_tables_follow_cli_contracts -q`
- Migration suite plus schema edge check: `UV_CACHE_DIR=/private/tmp/alab-uv-cache PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv --no-config run --locked pytest tests/test_migrations.py tests/test_runner_local.py::test_project_config_schema_maps_runner_reward_and_env_edges -q`
- Forced compile check: `UV_CACHE_DIR=/private/tmp/alab-uv-cache PYTHONPYCACHEPREFIX=/private/tmp/alab-pycache uv --no-config run --locked python -m compileall -q src tests`
- `git diff --check`

...
- 每个实际 hard remove command 都拒绝缺少 `--force`、缺少 `--confirm`、confirm id 错误、target 未 archived 的情况。
- 每个 hard remove command 都支持 `--dry-run`，输出 blocker 和 deletion count，且不写 audit row、不删除数据；target 未 archived 时输出稳定的 `target_not_archived` blocker 并 exit `0`。
- 每个 `--cascade` command 在存在 active dependent authoritative object 时失败，并输出稳定 blocker list。
...
- Migration 使用 ALAB_HOME-level file lock；project 和 experiment operations 使用 SQLite `locks` table。
- Global config defaults、invalid config repair-only behavior、field-level repair、`reset --all`，以及用于 Docker availability/platform/resource probes 的 `validate --refresh-capabilities`。
...
- Artifact capture errors 记录为 artifact statuses and warnings，且不改变 run/validation status。

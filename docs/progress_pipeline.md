...
- Next evidence to add: none by default during closeout. Only reopen implementation work if `docs/completion_audit.md` names a concrete defect or an explicit release target requires the remaining live SkyDiscover catalog gate.

## Active Queue

| Priority | Batch | Why it is next | Evidence target | Suggested focused checks |
| --- | --- | --- | --- | --- |
| P0 | Post-closeout audit-row decomposition | Only needed if continuing beyond the default/local runnable V1 closeout into exhaustive release evidence. Most recent default/fake-path proof families, including storage/audit/object retained relationships, core successful workflow, CLI golden/command-contract completeness, capability/help/payload preflight, source/public experiment direction, runner/adapter direction, host-support policy, lifecycle archive/unarchive/remove, and home/filesystem/path-registry mapping, already have direct evidence and should not be reopened without a named gap. | The remaining grouped blueprint/subsystem audit row, not broad already-proved families. | Add narrow tests or exact evidence references around one named audit edge at a time. |
| P1 | Remaining live catalog gate | Docker-backed Docker/Harbor/SkyDiscover Docker gates, real Docker capability refresh, and SkyDiscover Python local-wheel/network/native dependency gates passed on the current Darwin host. The only remaining real-environment gate is live SkyDiscover catalog reachability, which is currently blocked by GitHub SSL access from this environment. | Live SkyDiscover catalog row. | Re-run `ALAB_RUN_LIVE_SKYDISCOVER_CATALOG=1 ... pytest -m live_skydiscover_catalog -q -rs` on a network that can reach GitHub; rerun Docker/Python opt-in gates only if the release host/platform/Python environment differs. |

...
| --- | --- | --- | --- | --- |
| P0 | Post-closeout audit-row decomposition | Only needed if continuing beyond the default/local runnable V1 closeout into exhaustive release evidence. Most recent default/fake-path proof families, including lifecycle archive/unarchive/remove mapping, already have direct evidence and should not be reopened without a named gap. | Any remaining grouped `PARTIAL` row that names a concrete missing edge, not broad already-proved families. | Add narrow CLI/storage tests around one named audit edge at a time. |
| P0 | Full default-suite gate | Current worktree passed the default-suite closeout gate; rerun only after further implementation/test changes or immediately before a later release claim. | P0 gate in `docs/completion_audit.md` and gate snapshot in `docs/progress.md`. | `uv run pytest -q`, `uv run ruff check`, `python3 -m compileall -q src tests`, and `git diff --check`. |

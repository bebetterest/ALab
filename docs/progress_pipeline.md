...
## Current Active Batch - 2026-06-07

- Focus: the requested dashboard static presentation fixes and front-of-pipeline CI version synchronization gate are closed for the current worktree. The dashboard now renders the red best-so-far reward trend series from every scored run, carrying the prior best value forward when a later run does not set a new best, while preserving separate new-best marker/count behavior. Project detail sticky tabs also sit flush against the detail header with an opaque full-width background while scrolling, and run detail KPI rows reserve enough height for the horizontal scrollbar. CI now checks package version synchronization before lint, tests, opt-in gates, or publish jobs run.
- Explicit non-goals for this batch: do not change dashboard APIs, CLI command contracts, read-model pagination, storage schema, runner behavior, visibility rules, reward ranking semantics, package publishing semantics, or release asset upload semantics. This batch changes only static dashboard chart/detail/KPI layout presentation, the CI preflight ordering gate, and focused evidence.
...
- Evidence added: focused dashboard static frontend test, version synchronization script and CI ordering contract tests, dynamic release-note extraction test for the current-version changelog section, focused `ruff` for the touched Python test files, sandbox full-suite dashboard-bind failure confirmation, elevated full default suite, full-suite skip listing, opt-in skipped-test subset, `git diff --check`, and in-app Browser verification against `examples/dashboard_showcase/.run/alab-home` showing `SkyDiscover Circle Packing` #3 carrying the #2 best value forward, project detail tabs attaching to the header without a translucent gap, and `Embedding reranker` run detail KPI cards fully visible above the horizontal scrollbar.

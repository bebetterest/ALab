...
当前 free evaluation behavior 也包括成对的 `runner.type = "none"` 与 `reward.type = "none"` project configs、`not_required` project validation records、无需 evaluator run 的 direct experiment submit、nullable submission `final_run_id`、`final run id: none` 渲染、best-ranking exclusion，以及无 final run record 的目标 experiments 在 lifecycle/report/observe 中的覆盖。

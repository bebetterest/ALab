...
- Latest full default-suite closeout gate 已在 2026-06-04 project lifecycle service extraction and lazy compatibility-export review 中通过，覆盖将 project list/show/archive/unarchive/remove/locks handlers 移出 `src/alab/services.py` 后的当前 worktree。较早的 sandbox full suite 仅因为 dashboard tests 需要绑定 loopback ports 且 managed sandbox 以 `PermissionError` 拒绝而失败；elevated full default-suite validation 仍是本地 worktree 的 completion-quality gate。

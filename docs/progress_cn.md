...
当前 active focus 的短摘要：recently closed proof families 由 `docs/progress_closed_gaps.md` 跟踪，剩余 active queue 是 grouped audit-row decomposition 加 release gates。除非 `docs/completion_audit.md` 点名新的 edge，不要重启 security boundary negative proof、shared-runner cleanup、project config/schema mapping、hard-remove retained-row relationships、credential model proof mapping、maintenance/credential/token/context audit metadata、context marker conflict/alias mapping、observe visibility/filter/sort matrices 或 public `--from-exp` visibility-intersection proof 等已关闭工作。详细队列只放在 `docs/progress_pipeline.md`。
...
- 当前 worktree 的 full default-suite closeout gate 已通过；之后任何 source、test 或 documentation 变化都必须重跑。
...
| Full default suite | 2026-05-21 closeout 为 `PROVED` | 之后任何 source、test 或 documentation 变化后，都必须重新运行 `uv run pytest -q`、`uv run ruff check`、`python3 -m compileall -q src tests` 和 `git diff --check`。 |

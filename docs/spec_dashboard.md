...
- Project detail: overview, bounded recent experiments, runs, sources/config, validations, logs/artifacts, annotations, and audit records with related-record page totals. Overview must include project-level aggregate statistics. The runs tab must include a reward trend chart that plots each loaded run reward and connects each new best point according to the project's reward direction, plus one reference-metric trend chart for each declared `metrics.reference` entry when loaded runs contain numeric values for that metric.

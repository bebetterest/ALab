...
| --- | --- | --- | --- | --- |
| P0 | Post-closeout audit-row decomposition | 只有从 default/local runnable V1 closeout 继续进入 exhaustive release evidence 时才需要。最近的 default/fake-path proof families，包括 source/public experiment direction、runner/adapter direction、host-support policy、lifecycle archive/unarchive/remove 和 home/filesystem/path-registry mapping，已经有 direct evidence，除非有 named gap，否则不要重开。 | 任何仍点名具体缺失 edge 的 grouped `PARTIAL` row，而不是宽泛的已证明 family。 | 每次围绕一个 named audit edge 增加窄 CLI/storage tests。 |

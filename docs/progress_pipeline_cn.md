...
| --- | --- | --- | --- | --- |
| P0 | Post-closeout audit-row decomposition | 只有从 default/local runnable V1 closeout 继续进入 exhaustive release evidence 时才需要。最近的 default/fake-path proof families，包括 lifecycle archive/unarchive/remove mapping，已经有 direct evidence，除非有 named gap，否则不要重开。 | 任何仍点名具体缺失 edge 的 grouped `PARTIAL` row，而不是宽泛的已证明 family。 | 每次围绕一个 named audit edge 增加窄 CLI/storage tests。 |
| P0 | Full default-suite gate | 当前 worktree 已通过 default-suite closeout gate；只有后续 implementation/test changes 后，或之后 release claim 前才重跑。 | `docs/completion_audit.md` 的 P0 gate 和 `docs/progress.md` 的 gate snapshot。 | `uv run pytest -q`、`uv run ruff check`、`python3 -m compileall -q src tests` 和 `git diff --check`。 |
